...
let  tokenVault, tokenMint, vault, nonce, tokenVaultNonce, lpMint, lpMintNonce, adminLp, vaultLpToken, adminToken, userToken, userLp;
...
describe('staking', () => {
  ...
  it('Vault Is initialized', async () => {
    ...
    [tokenVault, tokenVaultNonce] = await anchor.web3.PublicKey.findProgramAddress(
      [
  ...
  it('Second token Staked', async () => {
    ...
    const userLpTokenAccount = await vaultLpToken.getAccountInfo(userLp);
    assert.strictEqual(tokenAmount+100_000_000, userLpTokenAccount.amount.toNumber());
  });
  ...
  it('Unstake', async () => {
    ...
    const userTokenAccount = await tokenMint.getAccountInfo(userToken);
    console.log(userTokenAccount.amount.toNumber()); // reward has been not released yet
  ...
  it('Update locked reward degradation', async () => { 
    ...
    assert.strictEqual(newRewardDegradation, vaultAccount.lockedRewardTracker.lockedRewardDegradation.toNumber());      
  ...
  it('Unstake after waiting', async () => {
    ...
    var tx = await program.rpc.unstake(
      new anchor.BN(lpAmount),
      {
        accounts: {
          vault,
